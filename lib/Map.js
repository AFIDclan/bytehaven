const { Entity, Rect, Image } = require("../engine");
...
class Map {
    constructor(engine, width, height) {
        ...
        this.wall_image = new Image("wall.png");
    ...
    load()
    {
        ...
        for (let x=bounds.start_x;x<bounds.end_x;x++)
        {
            for (let y=bounds.start_y;y<bounds.end_y;y++)
            {
                if (this.grid[x][y])
                {
                    let block = new Entity(this.wall_image, Rect.from_coordinates(0, 0, this.block_size, this.block_size), "DirtBlock");
