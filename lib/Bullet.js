const { PhysicsEntity } = require("../engine");
const { Polygon, Vector2 } = require("collidify");
const fs = require('fs');

//const bullet_svg = fs.readFileSync("./lib/bullet.svg", 'utf8');

class Bullet extends PhysicsEntity
{
    constructor(starting_pose, owner, name="Bullet")
    {
        let hitbox = new Polygon([
            new Vector2(0, 0),
            new Vector2(0, 20),
            new Vector2(20, 20),
            new Vector2(20, 0)
        ]);

        super("bullet1.png", hitbox, owner.name + "-" + name);

        this.set_position(starting_pose.x, starting_pose.y);
       this.rotate(starting_pose.angle);

        this.owner = owner;
        this.engine = owner.engine;

        this.muzzle_velocity = 50;
        this.time_to_live = 5000;
        this.damage = 10;

        //Calculate velocity
        this.velocity.x = this.muzzle_velocity * Math.sin(this._pose.angle);
        this.velocity.y = this.muzzle_velocity * -Math.cos(this._pose.angle);

        setTimeout(() => {
            this.die();
        }, this.time_to_live);
    }

    die()
    {
        this.engine.remove_entity(this);
    }

    on_collision(other_entity)
    {
        //Don't collide with owner
        if (other_entity.id != this.owner.id && other_entity.team_id != this.owner.team_id)
        {
            //Bullets don't collide with bullets
            if (other_entity instanceof Bullet)
                return;
            
            if (other_entity.take_damage)
                other_entity.take_damage(this.damage, this.owner);

            this.die();
        }
    }
}

module.exports = Bullet;